...
const allTasksCompany = async (req, res, next) => {
    try {
        var get = await Tasks.find({company: req.params.id})
            .populate('selectProject')
            .populate('addedby')


            console.log("get",get,  req.params.id)

        get && res.status(200).json({ message: "All Task", tasks: get })
    } catch (error) {
        next(error)
    }
}
